...
//   category?: string;    // Added to resolve modal issues
//   aspect?: string;
// }
...
// 1. Unified Prisma Task Interface matching TaskModal conventions
interface Task {
  id: string | number; 
  title: string;
  date: string;       // Used internally for mapping to calendar cells
  dueDate: string;     // The date string returned directly by Prisma/API
  priority: "high" | "medium" | "low" | string;
  category?: string;    
  aspect?: string;
}
...
export default function CalendarView() {
  ...
  const fetchTasks = async () => {
    try {
      ...
      // Normalize data safely with explicit mapping types
      const formatted = data.map((task: Task) => ({
        ...task,
        date: task.dueDate ? task.dueDate.split("T")[0] : "", 
      }));
  ...
  // 2. FIXED: Accept both string hashes and numbers to align with Prisma conventions
